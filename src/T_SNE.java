 ...
 */
public class T_SNE implements PlugIn {
    ...
    public void run(String arg) {
        ...
        if (WindowManager.getCurrentImage() != null && (WindowManager.getCurrentImage()).isStack()) {
            //GenericDialog gd = new GenericDialog("t-SNE option");	//consider replacing with purejava method
        	//gd.addMessage("Do you want to process the image stack?");
            	...
            	for (int s = 0; s < stack.getStackSize(); s++) {
                	...
                	for (int j = 0; j < height; j++) {
                		for (int k = 0; k < width; k++) {
                			image1DArray[k+(j*width)] = sip.getPixel(k, j);
                		}
                	}
    ...
    if (debugArray == null && !processingStack) {
        ...
        for (int i = 0; i < Filelist.length; i++) {
        	...
        	for (int j = 0; j < height; j++) {
        		for (int k = 0; k < width; k++) {
        			image1DArray[k+(j*width)] = ip.getPixel(k, j);
        		}
        	}
        	imp.close();
    	...
    	Plot scatter = new Plot("tSNE output", "tSNE_1", "tSNE_2");
    	if (labelsArray == null) {
    		scatter.addPoints(Xarray, Yarray, 0);
    		scatter.show();
    	} else {
    		...
    		for (int y = 0; y < groupN; y++) {
    			...
    			*/
    			...
    			scatter.addPoints(XXarray, YYarray, 0);
